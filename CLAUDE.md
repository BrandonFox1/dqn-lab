...
```bash
cd pacdqn && pip install -r requirements.txt && python -m pytest -q     # 42 tests: 41 pass, 1 skips unless gymnasium is installed (needs node)
...
pacdqn/                   Python project (own README with every CLI flag)
  ...
  runs/arcade_ft/         the lr 3e-5 fine-tune from runs/arcade's 57.5M brain; final.pt (65M) ships; combined/ = main run to 57.5M + fine-tune, the record the page's export uses
  runs/arcade_ft2/        the lr 1e-5 fine-tune from arcade_ft's 65M brain; final.pt (72.5M) ships; combined/ = the full chain, the record the page's export uses
...
- Four ghosts brain (from pacdqn/): `python -m pacdqn.arcade_train train --out runs/arcade --steps 30000000` · `... exam --run runs/arcade --games 50` · `... export --run runs/arcade_ft2/combined --ckpt final.pt --dest ../workshop/brains/arcade` (the shipped brain; then `npm run build` in workshop/)
...
- Four ghosts run (60M decisions in two segments, 10.8 h, 4 CPU cores; resumed at 30M from latest.pt with its optimizer): exam averages 13.7k (first 5M) → 25.3k (25–30M) → 28.4k (55–60M). The shipped `final.pt` (57.5M) won the 50-game selection on seeds 3000–3049 (31,721 vs 28,321 for the 30M brain). Its final check on 50 new games (seeds 2000–2049) scored 29,654 mean, 4.86 levels cleared per game, best game 48,000 (level 9); random play scores 718. A resumed run's `minutes` clock restarts at 0; export adds the segments (`total_minutes`).
- Four ghosts fine-tune (`runs/arcade_ft`): `--resume` from `runs/arcade/final.pt` (weights only, so Adam restarts) with `--lr 3e-5 --keep-every 2500000`, 57.5M → 67.5M decisions (1.2 h). Selection (seeds 3000–3049): 60M 32,300, 62.5M 32,214, **65M 36,336**, 65.5M best exam 32,808, 67.5M 32,632, vs 31,721 before. Final check of the shipped 65M brain: 33,720 mean, 6.06 levels cleared per game, best game 68,310 on level 15 (was 29,654 / 4.86). Clear `best` in the starting checkpoint (the trainer only saves best.pt when an exam beats the stored record).
- Second fine-tune (`runs/arcade_ft2`): from `runs/arcade_ft/final.pt` (65M) at `--lr 1e-5`, 65M → 75M. Selection: 67.5M 31,760, 70M 36,688, **72.5M 39,256**, 72.75M best exam 34,381, 75M 34,448, vs 36,336 before. Final check of the shipped 72.5M brain: 38,508 mean, 6.78 levels cleared per game, 41.0 ghosts per game, best game 62,780 on level 13 (was 33,720 / 6.06). About 11% better over all 100 test games.
